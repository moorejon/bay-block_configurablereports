...
class report_base {
    ...
    public function print_graphs($return = false) {
        ...
        if ($graphs) {
            foreach ($graphs as $g) {
                ...
                $output .= '<a target="_parent" href="'.$url->out().'"><img class="plot-image"src="'.$g.'" alt="'.$this->config->name.'"></a><br />';
                $output .= '</div>';
            }
        }
        if ($return) {
    ...
    public function print_export_options($return = false) {
        ...
        if (!empty($this->config->export)) {
            ...
            foreach ($export as $e) {
                if ($e) {
                    $output .= '<a href="'.$wwwpath.'&amp;download=1&amp;format='.$e.'"><img src="'.$CFG->wwwroot.'/blocks/configurable_reports/export/'.$e.'/'.$e.'.gif" alt="'.$e.'">&nbsp;'.(strtoupper($e)).'</a>&nbsp;';
                }
            }
            $output .= '</div>';
            if ($lastexport) {
    ...
    public function print_report_page(\moodle_page $moodlepage, $blockinstancecfg=null) {
        ...
        $this->config->blockinstancecfg = $blockinstancecfg;
        $chartonly = optional_param('chartonly', !empty($blockinstancecfg['displaychartonly']), PARAM_INT);
        if (!$chartonly) {
            $this->print_filters();
        }
