...
class plugin_courses extends plugin_base{
    ...
    public function execute($finalelements, $data) {
        ...
        if ($this->report->type != 'sql') {
            ...
            if (preg_match_all("/%%FILTER_COURSES:([^%]+)%%/i", $finalelements, $output)) {
                for ($i = 0; $i < count($output[1]); $i++) {
                    $replace = ' AND '.$output[1][$i].' = '.$filtercourses;
                    $finalelements = str_replace('%%FILTER_COURSES:'.$output[1][$i].'%%', $replace, $finalelements);
                }
            }
        }
