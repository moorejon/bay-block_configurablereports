...
class pie_form extends moodleform {
    public function definition() {
        ...
        for ($i = 0; $i < 5; $i++) {
            $mform->addElement('text', 'label'.$i, get_string('label', 'block_configurable_reports', $i));
            $mform->setType('label'.$i, PARAM_TEXT);

            $mform->addElement('text', 'labelcolor'.$i, get_string('labelcolor', 'block_configurable_reports', $i));
